const { subKeyFinder, textToBinary, binaryToText, groupingTexts, subKey, ip, bitSelect, sbox, p, ipInvers, binaryToHex } = require("./subKey.js");
...
function EncryptMessage(message, key) { 
  subKeyFinder(key);
  binaryValueMessage = textToBinary(message);
  ...
  for (let x = 0; x < messageGroup.length; x++) { 
    ...
    decryptedMessage = decryptedMessage.concat(binaryToText(decryptedBinaryMessage));
  }
  ...
  return binaryToHex(decryptedBinaryMessage);
